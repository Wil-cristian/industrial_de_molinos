...
class AnalyticsDataSource {
  ...
  static Future<List<CustomerMetrics>> getAllCustomerMetrics() async {
    try {
      ...
      try {
        ...
        AppLogger.warning(
          '?? Vista v_customer_metrics no existe, calculando manualmente...',
        );
  ...
  static Future<Map<String, dynamic>> getDashboardSummary() async {
    try {
      ...
      final invoicesPending = await _client
          .from('invoices')
          .select('total, paid_amount, sale_payment_type, delivery_date')
          .neq('status', 'paid')
          .neq('status', 'cancelled');
      ...
      // Procesar ventas del mes (excluir adelantos sin entregar)
      double monthlySales = 0;
      for (var inv in invoicesMonth) {
        if (inv['sale_payment_type'] == 'advance' && inv['delivery_date'] == null) continue;
        monthlySales += (inv['total'] as num).toDouble();
      }

      // Procesar cuentas por cobrar (excluir adelantos sin entregar)
  ...
  static Future<CollectionKPIs> getCollectionKPIs() async {
    try {
      ...
      final allInvoices = await _client
          .from('invoices')
          .select('id, total, paid_amount, issue_date, due_date, status, sale_payment_type, delivery_date')
      ...
      for (var inv in allInvoices) {
        // Adelantos sin entregar no cuentan como ventas ni cuentas por cobrar
        if (inv['sale_payment_type'] == 'advance' && inv['delivery_date'] == null) continue;
        countedInvoices++;
  ...
  static Future<Map<String, dynamic>?> refreshMaterializedViews() async {
    try {
      ...
      AppLogger.warning(
        '?? No se pudieron refrescar vistas materializadas: $e',
      );
      return null;
    }
  }
