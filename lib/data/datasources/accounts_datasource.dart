...
class AccountsDataSource {
  ...
  static Future<CashMovement> createMovement(CashMovement movement) async {
    ...
    // Obtener nombre de cuenta para auditoría
    String accountName = movement.accountId;
    try {
      final acct = await getAccountById(movement.accountId);
      accountName = acct?.name ?? movement.accountId;
    } catch (_) {}
    AuditLogDatasource.log(
      action: 'create',
      module: 'cash',
      ...
      details: {
        ...
        if (movement.personName != null) 'person_name': movement.personName,
      },
    );
    return created;
  }
    ...
    try {
      ...
      final result = await _client.rpc(
        ...
        params: {
          ...
          'p_date': ColombiaTime.toIso8601(date),
      ...
      // Obtener nombres de cuentas para auditoría
      String fromName = fromAccountId;
      String toName = toAccountId;
      try {
        final accounts = await Future.wait([
          getAccountById(fromAccountId),
          getAccountById(toAccountId),
        ]);
        fromName = accounts[0]?.name ?? fromAccountId;
        toName = accounts[1]?.name ?? toAccountId;
      } catch (_) {}
      ...
      AuditLogDatasource.log(
        ...
        recordId: outId,
        description:
            'Traslado: $fromName → $toName por \$${amount.toStringAsFixed(0)}',
        details: {
    ...
    try {
      final result = await _client.rpc(
        ...
        params: {
          ...
          'p_date': ColombiaTime.toIso8601(movement.date),
        },
      );
