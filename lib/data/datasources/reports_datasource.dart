...
class ReportsDataSource {
  ...
  static Future<SalesStats> getSalesStats({
    ...
    try {
      ...
      for (var invoice in currentResponse) {
        // Adelantos sin entregar no son ventas reales aún
        if (invoice['sale_payment_type'] == 'advance' && invoice['delivery_date'] == null) continue;
      ...
      final prevResponse = await _client
          .from('invoices')
          .select('total, sale_payment_type, delivery_date')
  ...
  static Future<double> _calculateGrossMargin(
    ...
    try {
      // Obtener items de facturas del período con cost_price
      final itemsResponse = await _client
          .from('invoice_items')
          .select(
            'quantity, unit_price, cost_price, invoice_id, invoices!inner(issue_date, status, sale_payment_type, delivery_date)',
          )
      ...
      for (var item in itemsResponse) {
        // Excluir items de facturas de adelanto sin entregar
        final invoice = item['invoices'] as Map<String, dynamic>?;
        if (invoice != null) {
          final spt = invoice['sale_payment_type']?.toString();
          final dd = invoice['delivery_date'];
          if (spt == 'advance' && dd == null) continue;
        }

        final qty = (item['quantity'] ?? 0).toDouble();
        final unitPrice = (item['unit_price'] ?? 0).toDouble();
        final costPrice = (item['cost_price'] ?? 0).toDouble();
        ...
        if (costPrice > 0) {
          ...
          // Items sin costo registrado: estimar con 65% del precio de venta
          totalCost += revenue * 0.65;
        }
      }
    ...
    try {
      ...
      final response = await _client
          ...
          .select('''
            ...
            total,
            invoice_id,
            invoices!inner(issue_date, status, sale_payment_type, delivery_date)
          ''')
          .gte('invoices.issue_date', ColombiaTime.dateString(startDate))
          .lte('invoices.issue_date', ColombiaTime.dateString(endDate))
          .neq('invoices.status', 'cancelled');
      ...
      for (var item in response) {
        // Excluir items de adelantos sin entregar
        final invoiceData = item['invoices'];
        if (invoiceData != null &&
            invoiceData['sale_payment_type'] == 'advance' &&
