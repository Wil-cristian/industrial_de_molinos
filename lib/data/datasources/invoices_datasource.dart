...
class InvoicesDataSource {
  ...
  static Future<void> updateStatus(String id, String status) async {
    ...
    const validTransitions = {
      ...
      'issued': ['paid', 'partial', 'cancelled', 'overdue'],
      'partial': ['paid', 'cancelled', 'overdue'],
      'overdue': ['paid', 'partial', 'cancelled'],
      'paid': ['cancelled'],
    };
    final allowed = validTransitions[currentStatus?.name] ?? [];
    if (currentStatus?.name == status) return; // Ya está en ese estado
    if (!allowed.contains(status)) {
      AppLogger.warning('⚠️ Transición inválida: ${currentStatus?.name} → $status');
      throw Exception(
        'Transición de estado no permitida: ${currentStatus?.name} → $status',
      );
    }
    ...
    // NO aplicar a facturas de tipo adelanto — su delivery_date se asigna
    // al confirmar entrega de la remisión.
    if ((status == 'issued' || status == 'paid') &&
        currentInvoice?.deliveryDate == null &&
        currentInvoice?.salePaymentType != 'advance') {
    ...
    // Si se está emitiendo el recibo, descontar stock (solo si no estaba ya emitido)
    if (status == 'issued' && currentStatus != InvoiceStatus.issued) {
    ...
    await AuditLogDatasource.log(
      ...
      details: {
        ...
        if (currentInvoice != null) ...{
          'invoice_number': '${currentInvoice.series}-${currentInvoice.number}',
          'customer': currentInvoice.customerName,
          'total': currentInvoice.total,
          'paid_amount': currentInvoice.paidAmount,
        },
      },
    );
  ...
  static Future<void> registerPayment({
    ...
    bool isSale = false, // true = venta nueva, false = abono/cobranza
  }) async {
    ...
    try {
      ...
      if (accountId != null && accountId.isNotEmpty) {
        final category = isSale ? MovementCategory.sale : MovementCategory.collection;

        // Construir descripción según tipo de movimiento
        String description;
        if (isSale && invoice.items.isNotEmpty) {
          // Venta nueva: mostrar productos vendidos
          final itemNames = invoice.items
              .map((i) => '${i.productName} x${i.quantity.toStringAsFixed(i.quantity == i.quantity.roundToDouble() ? 0 : 1)}')
              .join(', ');
          description = 'Venta ${invoice.series}-${invoice.number}: $itemNames';
        } else if (isSale) {
          description = 'Venta recibo ${invoice.series}-${invoice.number}';
        } else if (!isSale && invoice.items.isEmpty) {
          description = 'Pago deudas antiguas - ${invoice.series}-${invoice.number}';
        } else {
          // Abono/cobranza: indicar si es pago parcial o total
          final remaining = invoice.total - invoice.paidAmount;
          final isPartial = amount < remaining - 0.01;
          description = isPartial
              ? 'Abono recibo ${invoice.series}-${invoice.number}'
              : 'Cobro total recibo ${invoice.series}-${invoice.number}';
        }
      ...
      await AuditLogDatasource.log(
        ...
        recordId: invoiceId,
        description:
            'Abono de \$${amount.toStringAsFixed(0)} a factura ${invoice.series}-${invoice.number} (${invoice.customerName})',
        details: {
          'payment_amount': amount,
          'payment_method': method,
          'invoice_number': '${invoice.series}-${invoice.number}',
          'customer': invoice.customerName,
          'invoice_total': invoice.total,
          'previously_paid': invoice.paidAmount,
  ...
  static Future<Map<String, dynamic>> getMonthlyStats() async {
    ...
    double paidAmount = 0;
    double advanceNotDeliveredTotal = 0;
    double advanceNotDeliveredPaid = 0;
    int totalCount = 0;
    int paidCount = 0;
    int pendingCount = 0;
    int overdueCount = 0;
    int advanceNotDeliveredCount = 0;

    for (var inv in invoices) {
      // Separar adelantos sin entregar (no son ventas reales aún)
      final saleType = inv['sale_payment_type'] as String?;
      final deliveryDate = inv['delivery_date'];
      if (saleType == 'advance' && deliveryDate == null) {
        advanceNotDeliveredTotal += (inv['total'] as num).toDouble();
        advanceNotDeliveredPaid += (inv['paid_amount'] as num).toDouble();
        advanceNotDeliveredCount++;
        continue;
      }
    ...
    return {
      ...
      'advanceNotDeliveredTotal': advanceNotDeliveredTotal,
      'advanceNotDeliveredPaid': advanceNotDeliveredPaid,
      'advanceNotDeliveredCount': advanceNotDeliveredCount,
    };
  }
