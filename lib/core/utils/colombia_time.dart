...
class ColombiaTime {
  ...
  /// Si el equipo ya está en zona -05:00, se usa la hora local tal cual
  /// para evitar desfases dobles al guardar movimientos.
  static DateTime now() {
    final localNow = DateTime.now();
    if (localNow.timeZoneOffset == _offset) {
      return localNow;
    }
  ...
  static DateTime toColombia(DateTime date) {
    if (!date.isUtc && date.timeZoneOffset == _offset) {
      return date;
    }
    if (date.isUtc) {
