...
class _InvoicesPageState extends ConsumerState<InvoicesPage>
    ...
    return state.invoices
        .map(
          (inv) => {
            ...
            'isAdvanceNotDelivered': inv.isAdvanceNotDelivered,
  ...
  List<Map<String, dynamic>> get _filteredInvoices {
    return _invoices.where((invoice) {
      ...
      if (_selectedPaymentType == 'advance_not_delivered') {
        matchesPaymentType = invoice['isAdvanceNotDelivered'] == true;
      } else if (_selectedPaymentType != 'Todos') {
  ...
  // Adelantos sin entregar (no son deuda real)
  List<Map<String, dynamic>> get _advanceNotDeliveredInvoices =>
      _activeInvoices.where((inv) => inv['isAdvanceNotDelivered'] == true).toList();

  double get _totalVentas =>
      _activeInvoices.where((inv) => inv['isAdvanceNotDelivered'] != true)
          .fold(0.0, (sum, inv) => sum + (inv['total'] as double));
  double get _totalCobrado =>
      _activeInvoices.where((inv) => inv['isAdvanceNotDelivered'] != true)
          .fold(0.0, (sum, inv) => sum + (inv['paid'] as double));
  double get _totalAdelantos =>
      _advanceNotDeliveredInvoices.fold(0.0, (sum, inv) => sum + (inv['paid'] as double));
  double get _totalPendienteReal {
    // Pendiente total menos los adelantos de facturas sin entregar (esos no son deuda)
    final totalPending = _totalVentas - _totalCobrado;
    final advancePending = _advanceNotDeliveredInvoices.fold(
      0.0, (sum, inv) => sum + ((inv['total'] as double) - (inv['paid'] as double)),
    );
    return totalPending - advancePending;
  }
  double get _totalPendiente => _totalPendienteReal;
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            if (_advanceNotDeliveredInvoices.isNotEmpty) ...[
                              const SizedBox(width: 4),
                              _buildQuickStat(
                                '',
                                '${_advanceNotDeliveredInvoices.length} adel.',
                    ...
                    children: [
                      Wrap(
                        ...
                        children: [
                          ...
                          if (_advanceNotDeliveredInvoices.isNotEmpty)
                            _buildQuickStat(
                              'Adelantos (${_advanceNotDeliveredInvoices.length})',
                              Formatters.currency(_totalAdelantos),
                              const Color(0xFFE65100),
                              Icons.construction,
                            ),
                        ],
  ...
  Widget _buildInvoiceCard(Map<String, dynamic> invoice) {
    ...
    final isAdvanceNotDelivered = invoice['isAdvanceNotDelivered'] as bool? ?? false;

    // Color especial para adelantos sin entregar (morado/amber)
    final cardBorderColor = isAdvanceNotDelivered
        ? const Color(0xFFFF8F00) // Amber oscuro
        : Theme.of(context).colorScheme.outlineVariant.withValues(alpha: 0.5);
    final cardBgColor = isAdvanceNotDelivered
        ? const Color(0xFFFFF8E1) // Amber muy claro
        : null;
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: BorderSide(
          ...
          width: isAdvanceNotDelivered ? 1.5 : 1.0,
        ),
      ),
      child: InkWell(
        borderRadius: BorderRadius.circular(12),
        ...
        child: Padding(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // Badge adelanto sin entregar
              if (isAdvanceNotDelivered) ...[
                Container(
                  ...
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      const Icon(Icons.construction, size: 13, color: Color(0xFFE65100)),
                      const SizedBox(width: 4),
                      const Expanded(
                        child: Text(
                          'Adelanto - Pendiente de entrega (en producción)',
                          style: TextStyle(
                            fontSize: 11,
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      if (pending > 0 && invoice['status'] != 'Pagada')
                        Text(
                          isAdvanceNotDelivered
                              ? 'Adelanto: ${Formatters.currency(paid)}'
                              : 'Debe: ${Formatters.currency(pending)}',
                          style: TextStyle(
                            fontSize: 11,
                            color: isAdvanceNotDelivered
                                ? const Color(0xFFE65100)
                                : AppColors.warning,
                            fontWeight: FontWeight.w500,
                          ),
                  ...
                  Wrap(
                    ...
                    children:
                        [
                          ...
                          ('credit', 'Crédito'),
                          ('advance', 'Adelanto'),
                          ('advance_not_delivered', 'Adelanto sin entregar'),
                        ].map((e) {
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  if (isAdvanceNotDelivered) ...[
                    const SizedBox(width: 6),
                    const Icon(Icons.construction, size: 14, color: Color(0xFFE65100)),
                  ],
                ],
              ),
              Row(
...
class _InvoiceFullDetailDialogState
  ...
  Map<String, dynamic>? _linkedShipment;
  ...
  Future<void> _loadLinkedShipment() async {
    try {
      final invoiceId = widget.invoice['id']?.toString();
      if (invoiceId == null || invoiceId.isEmpty) return;
      final shipment = await ShipmentsDataSource.getByInvoiceId(invoiceId);
      if (mounted && shipment != null) {
        setState(() => _linkedShipment = shipment);
      }
    } catch (_) {}
  }

  Future<void> _showShipmentDetail(String shipmentId) async {
    // Cargar remisión completa con ítems
    ShipmentOrder? shipment;
    try {
      shipment = await ShipmentsDataSource.getById(shipmentId);
    } catch (_) {}
    ...
    if (shipment == null || !mounted) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No se pudo cargar la remisión'),
    ...
    if (!mounted) return;
    final s = shipment; // non-null local for closure
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: s.status == ShipmentStatus.entregada
                      ? const Color(0xFF2E7D32)
                      : const Color(0xFF1565C0),
              ...
              Flexible(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      // Info general
                      _shipmentInfoRow(Icons.person, 'Cliente', s.customerName),
                      if (s.customerAddress != null)
                        _shipmentInfoRow(Icons.location_on, 'Dirección', s.customerAddress!),
                      if (s.customerPhone != null)
                        _shipmentInfoRow(Icons.phone, 'Teléfono', s.customerPhone!),
                      if (s.invoiceFullNumber != null)
                        _shipmentInfoRow(Icons.receipt_long, 'Factura', s.invoiceFullNumber!),
                      if (s.productionOrderCode != null)
                        _shipmentInfoRow(Icons.precision_manufacturing, 'Orden Prod.', s.productionOrderCode!),
                      ...
                      // Fechas
                      const Text('Fechas', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                      const SizedBox(height: 8),
                      _shipmentInfoRow(Icons.calendar_today, 'Despacho', Helpers.formatDate(s.dispatchDate)),
                      if (s.deliveryDate != null)
                        _shipmentInfoRow(Icons.event, 'Entrega planificada', Helpers.formatDate(s.deliveryDate!)),
                      if (s.deliveredAt != null)
                        _shipmentInfoRow(Icons.check_circle_outline, 'Entregado', Helpers.formatDate(s.deliveredAt!)),

                      // Transporte
                      if (s.driverName != null || s.vehiclePlate != null) ...[
                        const SizedBox(height: 16),
                        const Divider(),
                        const SizedBox(height: 8),
                        const Text('Transporte', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                        const SizedBox(height: 8),
                        if (s.driverName != null)
                          _shipmentInfoRow(Icons.badge, 'Conductor', s.driverName!),
                        if (s.vehiclePlate != null)
                          _shipmentInfoRow(Icons.directions_car, 'Placa', s.vehiclePlate!),
                        if (s.carrierName != null)
                          _shipmentInfoRow(Icons.business, 'Transportadora', s.carrierName!),
                      ],

                      // Firmas
                      if (s.preparedBy != null || s.receivedBy != null) ...[
                        const SizedBox(height: 16),
                        const Divider(),
                        const SizedBox(height: 8),
                        const Text('Responsables', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                        const SizedBox(height: 8),
                        if (s.preparedBy != null)
                          _shipmentInfoRow(Icons.edit, 'Preparado por', s.preparedBy!),
                        if (s.approvedBy != null)
                          _shipmentInfoRow(Icons.approval, 'Aprobado por', s.approvedBy!),
                        if (s.receivedBy != null)
                          _shipmentInfoRow(Icons.handshake, 'Recibido por', s.receivedBy!),
                      ],
                      ...
                      if (s.items.isNotEmpty) ...[
                        ...
                        Row(
                          children: [
                            const Text('Ítems del Envío', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                            const SizedBox(width: 8),
                            Container(
                        ...
                        ...s.items.map((item) => Container(
                          ...
                          child: Row(
                            children: [
                              Icon(
                                item.itemType == ShipmentItemType.producto
                                    ? Icons.inventory_2
                                    : item.itemType == ShipmentItemType.material
                                        ? Icons.category
                                        : Icons.build,
                                size: 18,
                        ...
                        if (s.totalWeight > 0)
                          Padding(
                            ...
                            child: Text(
                              'Peso total: ${s.totalWeight.toStringAsFixed(1)} kg',
                      ...
                      if (s.notes != null && s.notes!.isNotEmpty) ...[
                        ...
                        const Divider(),
                        const SizedBox(height: 8),
                        const Text('Notas', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                        const SizedBox(height: 4),
                        Text(s.notes!, style: TextStyle(fontSize: 13, color: Colors.grey[700])),
                      ],
                    ],
                  ),
                ),
                ...
                child: Row(
                  ...
                  children: [
                    OutlinedButton.icon(
                      ...
                      label: const Text('Ir a Remisiones'),
                    ),
                    const SizedBox(width: 8),
                    FilledButton(
                      onPressed: () => Navigator.pop(ctx),
  ...
  Widget _shipmentInfoRow(IconData icon, String label, String value) {
    return Padding(
      ...
      child: SizedBox(
        ...
        child: Column(
          children: [
                            ...
                            Row(
                              children: [
                                ...
                                if (pending > 0) ...[
                                  ...
                                  Text(
                                    inv['isAdvanceNotDelivered'] == true
                                        ? 'Adelanto: ${Helpers.formatCurrency(paid)}'
                                        : 'Debe: ${Helpers.formatCurrency(pending)}',
                                    style: TextStyle(
                                      color: inv['isAdvanceNotDelivered'] == true
            ...
            // ── BANNER ADELANTO SIN ENTREGAR ──
            if (inv['isAdvanceNotDelivered'] == true)
              Container(
                width: double.infinity,
                ...
                child: Row(
                  children: [
                    const Icon(Icons.construction, size: 18, color: Color(0xFFE65100)),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          ...
                          Text(
                            'Este pago es un adelanto de trabajo. No se considera deuda hasta que el producto sea entregado.',
                            style: TextStyle(
                              fontSize: 11,
            ...
            // ── BANNER REMISIÓN ASOCIADA ──
            if (_linkedShipment != null)
              InkWell(
                onTap: () => _showShipmentDetail(_linkedShipment!['id']),
                child: Container(
                  width: double.infinity,
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                  color: _linkedShipment!['status'] == 'entregada'
                      ? const Color(0xFFE8F5E9)
                      : const Color(0xFFE3F2FD),
                  child: Row(
                    children: [
                      Icon(
                        _linkedShipment!['status'] == 'entregada'
                            ? Icons.check_circle
                            : Icons.local_shipping,
                        size: 18,
                        color: _linkedShipment!['status'] == 'entregada'
                      ...
                      Expanded(
                        child: Text(
                          'Remisión: ${_linkedShipment!['code']} — ${_shipmentStatusLabel(_linkedShipment!['status'])}',
                          style: TextStyle(
                            fontWeight: FontWeight.w600,
                            fontSize: 12,
                            color: _linkedShipment!['status'] == 'entregada'
            ...
            if (_linkedShipment == null && inv['status'] != 'Anulada')
              Container(
                width: double.infinity,
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                ...
                child: Row(
                  children: [
                    const Icon(Icons.warning_amber_rounded, size: 16, color: Color(0xFFF57F17)),
                    const SizedBox(width: 8),
                    Text(
                      'Sin remisión asociada — No se ha creado una remisión para esta factura',
                      style: TextStyle(
                        fontSize: 11,
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                final costPriceVal = (prod['costPrice'] as num?)?.toDouble() ?? 0;
