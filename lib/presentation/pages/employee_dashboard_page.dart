...
/// Dashboard read-only para empleados — muestra su info, horas, pagos, ausencias y préstamos
...
class _EmployeeDashboardPageState extends ConsumerState<EmployeeDashboardPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final employeeId = roleState.employeeId;

      if (employeeId == null) {
        setState(() {
          _error =
              'Tu cuenta no tiene un empleado vinculado. Contacta al administrador.';
          _isLoading = false;
        });
        return;
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: false,
        actions: [
          if (roleState.profile != null)
            Padding(
              padding: const EdgeInsets.only(right: AppSpacing.sm),
              child: Chip(
                avatar: Icon(Icons.badge, size: 16, color: cs.primary),
                label: Text(
                  roleState.profile!.displayName ?? 'Empleado',
            ...
            Wrap(
              ...
              children: [
                _infoChip(
                  Icons.badge,
                  'Doc',
                  '${emp.documentType ?? ''} ${emp.documentNumber ?? 'N/A'}',
                  tt,
                  cs,
                ),
                ...
                _infoChip(
                  ...
                  'Ingreso',
                  _dateFormat.format(emp.hireDate),
                  tt,
                  cs,
                ),
                if (emp.salary != null)
                  _infoChip(
                    Icons.attach_money,
                    'Salario',
                    _currencyFormat.format(emp.salary),
                    tt,
                    cs,
                  ),
                _infoChip(
  ...
  Widget _buildHoursCard(ColorScheme cs, TextTheme tt) {
    // Resumen de horas recientes
    final recentEntries = _timeEntries.take(10).toList();
    final totalWorked = _timesheets.fold<int>(
      0,
      (sum, ts) => sum + ts.workedMinutes,
    );
    final totalOvertime = _timesheets.fold<int>(
      0,
      (sum, ts) => sum + ts.overtimeMinutes,
    );
  ...
  Widget _timeEntryTile(EmployeeTimeEntry entry, TextTheme tt, ColorScheme cs) {
    final checkIn = entry.checkIn != null
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          Text(
            _formatMinutes(entry.workedMinutes),
          ...
          if (entry.overtimeMinutes > 0) ...[
            ...
            Container(
              ...
              child: Text(
                '+${_formatMinutes(entry.overtimeMinutes)}',
  ...
  Widget _payrollTile(EmployeePayroll payroll, TextTheme tt, ColorScheme cs) {
    final statusColor = payroll.status == 'pagada'
        ? AppColors.success
        : payroll.status == 'aprobada'
        ? AppColors.info
        : cs.onSurfaceVariant;
    final statusLabel = payroll.status == 'pagada'
        ? 'Pagada'
        : payroll.status == 'aprobada'
        ? 'Aprobada'
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(
            child: Text(
              payroll.periodName ?? 'Período',
              style: tt.bodySmall?.copyWith(fontWeight: FontWeight.w500),
            ),
          ),
          Text(
            _currencyFormat.format(payroll.netPay),
            style: tt.bodySmall?.copyWith(fontWeight: FontWeight.w600),
          ),
          const SizedBox(width: AppSpacing.sm),
          Container(
            ...
            child: Text(
              statusLabel,
              style: tt.labelSmall?.copyWith(color: statusColor, fontSize: 10),
            ),
          ),
        ],
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            if (_incapacities.isEmpty)
              ...
              ..._incapacities
                  .take(6)
                  .map((inc) => _incapacityTile(inc, tt, cs)),
          ],
        ),
      ),
    );
  ...
  Widget _incapacityTile(EmployeeIncapacity inc, TextTheme tt, ColorScheme cs) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Icon(
            active ? Icons.circle : Icons.check_circle,
            size: 10,
          ...
          Text(
            '${_dateFormat.format(inc.startDate)} - ${_dateFormat.format(inc.endDate)}',
            style: tt.labelSmall?.copyWith(color: cs.onSurfaceVariant),
          ),
          const SizedBox(width: AppSpacing.sm),
          Text(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text(
                  loan.reason ?? 'Préstamo',
                  style: tt.bodySmall?.copyWith(fontWeight: FontWeight.w500),
                ),
              ),
              Text(
                _currencyFormat.format(loan.totalAmount),
                style: tt.bodySmall?.copyWith(fontWeight: FontWeight.w600),
              ),
              const SizedBox(width: AppSpacing.sm),
              Container(
