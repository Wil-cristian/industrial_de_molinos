...
class _NewSalePageState extends ConsumerState<NewSalePage> {
  ...
  int _creditWeeks = 4;
  DateTime _creditStartDate = ColombiaTime.now();
  ...
  Future<void> _confirmSale() async {
    ...
    try {
      ...
      // Fecha de entrega (null para adelantos — se marca al entregar)
      final deliveryDate = _paymentType == 'advance'
          ? null
          : ColombiaTime.now().add(
              Duration(days: int.tryParse(_deliveryDaysController.text) ?? 3),
            );

      // Días de crédito efectivos (se almacenan para recalcular al entregar)
      final int effectiveCreditDays = _paymentType == 'credit'
          ? (_creditFrequency == 'weeks'
              ? _creditWeeks * 7
              : _creditDays)
          : _paymentType == 'advance'
      ...
      // Fecha de vencimiento: crédito empieza desde la ENTREGA, no la emisión
      final DateTime dueDate;
      if (_paymentType == 'credit') {
        if (_creditFrequency == 'weeks') {
          // Semanas: el usuario escoge fecha inicio manualmente
          dueDate = _creditStartDate.add(Duration(days: _creditWeeks * 7));
        } else {
          // Días: vence N días después de la entrega estimada
          final base = deliveryDate ?? ColombiaTime.now();
          dueDate = base.add(Duration(days: _creditDays));
        }
      } else if (_paymentType == 'advance') {
        // Adelanto: estimación provisional (se recalcula al confirmar entrega)
        dueDate = ColombiaTime.now().add(Duration(days: _advanceCreditDays));
      } else {
        dueDate = ColombiaTime.now(); // Contado: pago inmediato
      }
      ...
      final invoice = await InvoicesDataSource.createWithItems(
        ...
        creditDays: effectiveCreditDays,
        taxRate: 0,
        discount: _discountAmount,
        laborCost: _laborCost,
        items: _items.map((item) {
          ...
          final itemCostPrice = (item['costPrice'] as num?)?.toDouble() ?? 0;
          return InvoiceItem(
            id: '',
            invoiceId: '',
