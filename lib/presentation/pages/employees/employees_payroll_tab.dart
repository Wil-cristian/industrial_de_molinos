...
Map<String, dynamic>? resolveAdjacentPayrollPeriod({
  required int currentYear,
  required int currentPeriodNumber,
  required bool moveForward,
  DateTime? now,
}) {
  final referenceDate = now ?? ColombiaTime.now();
  final maxAllowedPeriod = referenceDate.day <= 15
      ? (referenceDate.month * 2 - 1)
      : (referenceDate.month * 2);
  final maxAllowedYear = referenceDate.year;

  var targetYear = currentYear;
  var targetPeriodNumber = currentPeriodNumber + (moveForward ? 1 : -1);

  if (targetPeriodNumber < 1) {
    targetYear -= 1;
    targetPeriodNumber = 24;
  } else if (targetPeriodNumber > 24) {
    targetYear += 1;
    targetPeriodNumber = 1;
  }

  if (moveForward) {
    final goesBeyondCurrent =
        targetYear > maxAllowedYear ||
        (targetYear == maxAllowedYear &&
            targetPeriodNumber > maxAllowedPeriod);
    if (goesBeyondCurrent) return null;
  }

  return {
    'year': targetYear,
    'periodNumber': targetPeriodNumber,
    'isQ1': targetPeriodNumber.isOdd,
  };
}
...
class EmployeesPayrollTabState extends ConsumerState<EmployeesPayrollTab> {
  ...
  Future<void> _changeDisplayedPeriod(bool moveForward) async {
    final notifier = ref.read(payrollProvider.notifier);
    var payrollState = ref.read(payrollProvider);
    var currentPeriod = payrollState.currentPeriod;

    if (currentPeriod == null) {
      await notifier.loadAll();
      payrollState = ref.read(payrollProvider);
      currentPeriod = payrollState.currentPeriod;
      if (currentPeriod == null) return;
    }

    final targetInfo = resolveAdjacentPayrollPeriod(
    ...
    if (targetInfo == null) {
      if (mounted && moveForward) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Ya estás en la quincena más reciente disponible.'),
          ),
        );
      }
      return;
    ...
    final int targetYear = targetInfo['year'] as int;
    final int targetPeriodNumber = targetInfo['periodNumber'] as int;

    PayrollPeriod? targetPeriod = payrollState.periods
        .where(
          (p) =>
              p.periodType == 'quincenal' &&
              p.year == targetYear &&
              p.periodNumber == targetPeriodNumber,
        )
        .firstOrNull;

    if (targetPeriod == null) {
      final bool isQ1 = targetPeriodNumber.isOdd;
      final int targetMonth = isQ1
          ? (targetPeriodNumber + 1) ~/ 2
          : targetPeriodNumber ~/ 2;
      final DateTime startDate = isQ1
          ? DateTime(targetYear, targetMonth, 1)
          : DateTime(targetYear, targetMonth, 16);
      final DateTime endDate = isQ1
          ? DateTime(targetYear, targetMonth, 15)
          : DateTime(targetYear, targetMonth + 1, 0);

      targetPeriod = await PayrollDatasource.createPeriod(
    ...
    if (targetPeriod != null) {
      await notifier.loadPayrollsForPeriod(targetPeriod.id);
    }
  }
                ...
                return Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        IconButton(
                          onPressed: () => _changeDisplayedPeriod(false),
                        ...
                        IconButton(
                          onPressed: () => _changeDisplayedPeriod(true),
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) {
          final isMobileDialog = ResponsiveHelper.isMobile(context);
          ...
          return AlertDialog(
            ...
            title: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    isComplemento ? 'Pago Complementario' : 'Crear Nómina',
                    overflow: TextOverflow.ellipsis,
                  ),
                ),
              ],
