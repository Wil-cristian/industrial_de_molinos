...
class _SaleInvoiceScanDialogState extends ConsumerState<SaleInvoiceScanDialog> {
  ...
  Future<void> _saveOneInvoice(_BatchItem item) async {
    ...
    final invoiceItems = item.itemMatches.map((im) {
      ...
      // Obtener costo del producto/material emparejado
      double itemCostPrice = 0;
      if (im.matchedProduct != null) {
        itemCostPrice = im.matchedProduct!.costPrice;
      } else if (im.matchedMaterial != null) {
        itemCostPrice = im.matchedMaterial!.effectiveCostPrice;
      }
      return InvoiceItem(
        id: '',
        invoiceId: '',
